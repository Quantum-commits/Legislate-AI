...
export const generatePDF = (data: DocumentData): jsPDF => {
  ...
  const headerText = data.type === 'FIR' ? 
    (data.language === 'hi' ? 'प्राथमिकी रिपोर्ट (FIR)' : 
     ...
     data.language === 'kn' ? 'ಪ್ರಾಥಮಿಕ ಮಾಹಿತಿ ವರದಿ (FIR)' :
     'First Information Report (FIR)') :
    (data.language === 'hi' ? 'सूचना का अधिकार आवेदन (RTI)' :
     data.language === 'te' ? 'సమాచార హక్కు దరఖాస్తు (RTI)' :
     data.language === 'mr' ? 'माहिती अधिकार अर्ज (RTI)' :
     data.language === 'kn' ? 'ಮಾಹಿತಿ ಹಕ್ಕು ಅರ್ಜಿ (RTI)' :
...
export const sharePDFOnWhatsApp = (data: DocumentData) => {
  ...
  const headerText = data.type === 'FIR' ? 
    (data.language === 'hi' ? '🏛️ प्राथमिकी रिपोर्ट (FIR)\n' : 
     data.language === 'te' ? '🏛️ ప్రాథమిక నివేదిక (FIR)\n' :
     data.language === 'mr' ? '🏛️ प्राथमिक अहवाल (FIR)\n' :
     data.language === 'kn' ? '🏛️ ಪ್ರಾಥಮಿಕ ಮಾಹಿತಿ ವರದಿ (FIR)\n' :
     '🏛️ First Information Report (FIR)\n') :
    (data.language === 'hi' ? '📋 सूचना का अधिकार आवेदन (RTI)\n' :
     data.language === 'te' ? '📋 సమాచార హక్కు దరఖాస్తు (RTI)\n' :
     data.language === 'mr' ? '📋 माहिती अधिकार अर्ज (RTI)\n' :
     data.language === 'kn' ? '📋 ಮಾಹಿತಿ ಹಕ್ಕು ಅರ್ಜಿ (RTI)\n' :
  ...
  const message = `${headerText}\n📅 Date: ${new Date().toLocaleDateString()}\n\n${data.content}\n\n⚖️ Generated by Legislate AI Legal Assistant`;
